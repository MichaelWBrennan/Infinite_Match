...
class RealtimeCalendarService {
    ...
    async createCalendarEvent(eventData) {
        try {
            const event = {
                ...
                start_time: moment.tz(eventData.startTime, eventData.timezone || this.timezone).utc().toISOString(),
                end_time: moment.tz(eventData.endTime, eventData.timezone || this.timezone).utc().toISOString(),
    ...
    async updateCalendarEvent(eventId, updateData) {
        try {
            ...
            if (updateData.startTime) {
                updateFields.start_time = moment.tz(updateData.startTime, updateData.timezone || this.timezone).utc().toISOString();
            }
            if (updateData.endTime) {
                updateFields.end_time = moment.tz(updateData.endTime, updateData.timezone || this.timezone).utc().toISOString();
            }
    ...
    processCalendarEvent(event, targetTimezone) {
        return {
            ...
            startTime: moment.tz(event.start_time, 'UTC').tz(targetTimezone).format('YYYY-MM-DD HH:mm:ss'),
            ...
            duration: moment.tz(event.end_time, 'UTC').diff(moment.tz(event.start_time, 'UTC'), 'minutes'),
        ...
        try {
            ...
            if (nextOccurrence && now.isAfter(nextOccurrence)) {
                ...
                const newEvent = {
                    ...
                    end_time: nextOccurrence.clone().add(event.duration || 60, 'minutes').utc().toISOString(),
                    created_at: new Date().toISOString(),
                    updated_at: new Date().toISOString()
                };
    ...
    async handleMonthlyReset() {
        try {
            ...
            const monthlyResetEvent = {
                ...
                metadata: {
                    resetType: 'monthly',
                    rewards: ['monthly_energy', 'monthly_coins', 'monthly_gems', 'monthly_items', 'seasonal_rewards']
                }
            };
    ...
    async getCalendarStatistics() {
        try {
            ...
            for (const event of data) {
                ...
                } else if (eventStart.isBefore(now) && moment.tz(event.start_time, 'UTC').add(1, 'hour').isAfter(now)) {
                    stats.ongoingEvents++;
                }
            }
