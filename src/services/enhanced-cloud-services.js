...
class EnhancedCloudServicesManager {
    ...
    async loadGameState(playerId) {
        ...
        if (this.mongodb && this.circuitBreakers.get('mongodb').state !== 'OPEN') {
            try {
                ...
                const gameState = await db.collection('game_states')
        ...
        const cloudServices = [
            { name: 'aws', load: () => this.loadGameDataFromS3(process.env.AWS_S3_BUCKET, `game-states/${playerId}/latest.json`) },
            { name: 'google', load: () => this.loadGameDataFromGCS(process.env.GOOGLE_CLOUD_BUCKET, `game-states/${playerId}/latest.json`) },
            { name: 'azure', load: () => this.loadGameDataFromAzureBlob(process.env.AZURE_CONTAINER_NAME, `game-states/${playerId}/latest.json`) }
        ];
    ...
    async saveGameDataToMongoDB(playerId, gameData) {
        const db = this.mongodb.db('match3_analytics');
        await db.collection('game_states').replaceOne(
            { playerId },
            gameData,
            { upsert: true }
        );
