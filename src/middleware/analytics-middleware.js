...
export const analyticsMiddleware = (req, res, next) => {
    ...
    // Track request start
    analyticsService.trackGameEvent('api_request_started', {
        method: req.method,
        url: req.url,
    ...
    res.end = function(chunk, encoding) {
        ...
        analyticsService.trackGameEvent('api_request_completed', {
            method: req.method,
            url: req.url,
...
export const errorTrackingMiddleware = (error, req, res, next) => {
    ...
    cloudServices.sendGameEventNotification('error_occurred', req.user?.id, {
        error: {
            name: error.name,
            message: error.message,
            stack: error.stack
...
export const gameEventMiddleware = (req, res, next) => {
    ...
    if (req.path.startsWith('/api/game/')) {
        ...
        // Track game event
        analyticsService.trackGameEvent(`game_${eventType}`, {
            ...req.body,
            request_id: req.headers['x-request-id'],
