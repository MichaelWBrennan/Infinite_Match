...
class UnityGamingServicesHeadlessIntegration {
  ...
  async deployEconomy() {
    ...
    try {
      ...
      console.log(
        `      - Currencies: ${results.currencies.created} created, ${results.currencies.errors} errors`,
      );
      console.log(
        `      - Inventory: ${results.inventory.created} created, ${results.inventory.errors} errors`,
      );
      console.log(
        `      - Catalog: ${results.catalog.created} created, ${results.catalog.errors} errors`,
      );
    } catch (error) {
  ...
  generateDeploymentSummary(results) {
    ...
    console.log(
      `   Success Rate: ${totalOperations > 0 ? ((successfulOperations / totalOperations) * 100).toFixed(1) : 0}%`,
    );
