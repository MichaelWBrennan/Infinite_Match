...
router.post('/level-complete', [
  ...
  body('starsEarned').isInt({ min: 0, max: 3 }).withMessage('Stars earned must be between 0 and 3'),
...
router.post('/match-made', [
  ...
  body('piecesMatched').isInt({ min: 1 }).withMessage('Pieces matched must be a positive integer'),
  body('position').isObject().withMessage('Position must be an object'),
  body('level').isInt({ min: 1 }).withMessage('Level must be a positive integer'),
  body('scoreGained').isInt({ min: 0 }).withMessage('Score gained must be a non-negative integer'),
], validateRequest, async (req: Request, res: Response) => {
  try {
