...
class HealthChecker:
        ...
        if config_path.exists():
            try:
                with open(config_path, "r") as f:
                    ...
                    status["environment_id_configured"] = bool(
    ...
    def check_recent_activity(self):
        ...
        status = {"recent_commits": 0, "recent_builds": 0, "last_activity": None}
        ...
        for check_name, weight in weights.items():
            if check_name in checks:
                ...
                if check_name == "economy_data":
                    score = (
                        1
                        if check_status.get("csv_valid", False)
                        and check_status.get("json_valid", False)
                        else 0
                    )
                elif check_name == "unity_services":
                    score = (
                        1
                        if check_status.get("config_valid", False)
                        and check_status.get("project_id_configured", False)
                        else 0
                    )
                    ...
                    score = (
                        1
                        if check_status.get("python_available", False)
                        and check_status.get("git_available", False)
                        else 0
                    )
                elif check_name == "activity":
                    score = (
                        1
                        if check_status.get("recent_commits", 0) > 0
                        or check_status.get("recent_builds", 0) > 0
                        else 0
                    )
                else:
                    score = 0
    ...
    def generate_health_report(self):
        ...
        if (
            not self.health_status["checks"]
            .get("unity_services", {})
            .get("project_id_configured", False)
        ):
            recommendations.append(
                "Configure Unity Services - update project ID and environment ID"
            )
        ...
        if (
            self.health_status["checks"].get("activity", {}).get("recent_commits", 0)
            == 0
        ):
            recommendations.append(
                "No recent activity - consider running maintenance tasks"
            )
