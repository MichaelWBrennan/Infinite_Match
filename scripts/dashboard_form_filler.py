...
// Currency data
const currencies = """ + json.dumps(currencies, indent=2) + """;
...
const inventoryItems = """ + json.dumps(inventory_items, indent=2) + """;
...
const virtualPurchases = """ + json.dumps(virtual_purchases, indent=2) + """;
...
"""
    ...
    def generate_all_fillers(self):
        ...
        currency_js = self.generate_currency_filler(config["services"]["economy"]["currencies"])
        currency_path = self.repo_root / "dashboard_currency_filler.js"
        with open(currency_path, 'w') as f:
            f.write(currency_js)
        
        # Generate inventory filler
        inventory_js = self.generate_inventory_filler(config["services"]["economy"]["inventoryItems"])
        inventory_path = self.repo_root / "dashboard_inventory_filler.js"
        with open(inventory_path, 'w') as f:
            f.write(inventory_js)
        
        # Generate purchase filler
        purchase_js = self.generate_purchase_filler(config["services"]["economy"]["virtualPurchases"])
