...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/package-updater.log"), logging.StreamHandler()],
)
...
class UnityPackageUpdater:
    ...
    def check_for_updates(self) -> List[PackageInfo]:
        ...
        for package_name, current_version in self.current_manifest.get(
            "dependencies", {}
        ).items():
    ...
    def create_backup(self) -> str:
        ...
        subprocess.run(
            [
                "cp",
                "-r",
                str(self.unity_project_path / "ProjectSettings"),
                str(backup_dir),
            ],
            check=True,
        )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "-executeMethod",
                    "Evergreen.Editor.BuildScript.TestBuild",
                ],
                capture_output=True,
                text=True,
                timeout=300,
    ...
    def rollback(self, backup_dir: str) -> bool:
        ...
        try:
            ...
            subprocess.run(
                [
                    "cp",
                    str(Path(backup_dir) / "manifest.json"),
            ...
            subprocess.run(
                [
                    "cp",
                    "-r",
                    str(Path(backup_dir) / "ProjectSettings"),
                    str(self.unity_project_path),
                ],
                check=True,
            )
    ...
    def generate_report(self, updates: List[UpdateResult]) -> str:
        ...
        report = {
            ...
            "successful_updates": len(
                [u for u in updates if u.status == UpdateStatus.SUCCESS]
            ),
            "failed_updates": len(
                [u for u in updates if u.status == UpdateStatus.FAILED]
            ),
            "skipped_updates": len(
                [u for u in updates if u.status == UpdateStatus.SKIPPED]
            ),
            "updates": [
                {
                    "package": u.package,
        ...
        try:
            ...
            for update in available_updates:
                ...
                if update.update_type == "major" and update.breaking_changes:
                    logger.warning(
                        f"Skipping {update.name} - major update with breaking changes"
                    )
