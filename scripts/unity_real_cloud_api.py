#!/usr/bin/env python3
"""
Unity Cloud Real API Automation
Populates Unity Cloud dashboard using real Unity Cloud API calls
Requires valid Unity Cloud API credentials
"""
...
class UnityCloudRealAPI:
    def __init__(self):
        ...
        if not self.api_token and not (self.client_id and self.client_secret):
            raise ValueError("Unity Cloud API credentials not found. Set UNITY_API_TOKEN or UNITY_CLIENT_ID/UNITY_CLIENT_SECRET")
        ...
        for currency in currencies:
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(f"   ⚠️ Currency already exists: {currency['name']} ({currency['id']})")
                    created_currencies.append(currency)
                else:
                ...
                print(f"   ❌ Error creating currency {currency['id']}: {e}")
        ...
        for item in inventory_items:
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(f"   ⚠️ Inventory item already exists: {item['name']} ({item['id']})")
                    created_items.append(item)
                else:
                ...
                print(f"   ❌ Error creating inventory item {item['id']}: {e}")
        ...
        for purchase in virtual_purchases:
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(f"   ⚠️ Virtual purchase already exists: {purchase['name']} ({purchase['id']})")
                    created_purchases.append(purchase)
                else:
                ...
                print(f"   ❌ Error creating virtual purchase {purchase['id']}: {e}")
            ...
            try:
                ...
                if os.path.exists(function_path):
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        print(f"   ⚠️ Cloud Code function already exists: {function}")
                        deployed_functions.append(function)
                    else:
                ...
                print(f"   ❌ Error deploying function {function}: {e}")
    ...
    def generate_deployment_report(self, results):
        ...
        report = {
            ...
            "deployment_type": "real_api",
            "results": results
        }
        
        report_path = "unity_cloud_real_deployment_report.json"
    ...
    def run_real_deployment(self):
        """Run complete real API deployment"""
        print("🚀 Starting Unity Cloud Real API Deployment")
        print("=" * 60)
        ...
        # Summary
        print("\n🎉 Unity Cloud Real API Deployment Complete!")
        print("=" * 60)
...
if __name__ == "__main__":
    try:
        deployment = UnityCloudRealAPI()
        deployment.run_real_deployment()
    except ValueError as e:
        print(f"❌ Configuration error: {e}")
        print("\nTo use real Unity Cloud API, set one of:")
        print("  UNITY_API_TOKEN=your_api_token")
        print("  UNITY_CLIENT_ID=your_client_id")
        print("  UNITY_CLIENT_SECRET=your_client_secret")
        print("\nOr run the mock version:")
        print("  python3 scripts/unity_headless_cloud_api.py")