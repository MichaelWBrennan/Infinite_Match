...
class PathReferenceUpdater:
    ...
    def update_using_statements(self, content):
        ...
        content = re.sub(
            r"using\s+Evergreen\.Core\.GameManager;", "using Evergreen.Core;", content
        )
        ...
        content = re.sub(
            r"using\s+Evergreen\.Architecture;", "using Evergreen.Core;", content
        )

        # Update UI system using statements
        content = re.sub(
            r"using\s+Evergreen\.UI\.EnhancedUIManager;", "using Evergreen.UI;", content
        )

        content = re.sub(
            r"using\s+Evergreen\.UI\.AdvancedUIManager;", "using Evergreen.UI;", content
        )

        # Update game system using statements
        content = re.sub(
            r"using\s+Evergreen\.Game\.Board;", "using Evergreen.Game;", content
        )

        content = re.sub(
            r"using\s+Evergreen\.Game\.LevelManager;", "using Evergreen.Game;", content
        )
    ...
    def update_class_references(self, content):
        ...
        content = re.sub(
            r"\bGameManager\.Instance\b", "OptimizedCoreSystem.Instance", content
        )

        content = re.sub(
            r"\bServiceLocator\.Get<", "OptimizedCoreSystem.Instance.Resolve<", content
        )
        ...
        content = re.sub(
            r"\bEnhancedUIManager\.Instance\b", "OptimizedUISystem.Instance", content
        )

        content = re.sub(
            r"\bAdvancedUISystem\.Instance\b", "OptimizedUISystem.Instance", content
        )

        # Update game system references
        content = re.sub(
            r"\bBoard\.Instance\b", "OptimizedGameSystem.Instance", content
        )

        content = re.sub(
            r"\bLevelManager\.Instance\b", "OptimizedGameSystem.Instance", content
        )
        ...
        content = re.sub(
            r"\bEnergySystem\.Instance\b", "OptimizedGameSystem.Instance", content
        )
