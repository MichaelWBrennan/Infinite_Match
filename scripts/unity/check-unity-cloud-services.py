...
class UnityCloudServicesChecker:
    ...
    def check_service_endpoints(self):
        ...
        services_to_check = {
            ...
            "Analytics": [f"{self.base_url}/projects/{self.project_id}/analytics"],
        }
        ...
        for service_name, endpoints in services_to_check.items():
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if self.check_service_content(response.text, service_name):
                            ...
                            print(
                                f"   ✅ {endpoint}: Accessible but service may not be configured"
                            )
                        ...
                        print(
                            f"   ❌ {endpoint}: Not found (404) - Service may not be enabled"
                        )
                    else:
            ...
            if accessible_endpoints > 0:
                ...
                print(
                    f"   📊 {service_name}: ACCESSIBLE ({accessible_endpoints} endpoints)"
                )
                ...
                print(
                    f"   📊 {service_name}: AUTH REQUIRED ({auth_required_endpoints} endpoints)"
                )
            elif not_found_endpoints > 0:
                service_result["status"] = "not_enabled"
                service_result["enabled"] = False
                print(
                    f"   📊 {service_name}: NOT ENABLED ({not_found_endpoints} endpoints)"
                )
            else:
    ...
    def generate_services_report(self):
        ...
        print(
            f"   Success Rate: {((enabled_services + accessible_services)/total_services)*100:.1f}%"
        )
