...
class HeadlessCSharpCloudCodeDeployer:
    ...
    def navigate_to_cloud_code(self):
        ...
        WebDriverWait(self.driver, 20).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, "[data-testid='project-navigation']")
            )
        )
        ...
        try:
            ...
            cloud_code_link = self.driver.find_element(
                By.XPATH, "//a[contains(text(), 'Cloud Code')]"
            )
            cloud_code_link.click()
        ...
        WebDriverWait(self.driver, 20).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, "[data-testid='cloud-code']")
            )
        )
    ...
    def deploy_csharp_functions(self):
        ...
        for cs_file in csharp_dir.glob("*.cs"):
            if cs_file.name != "ValidationUtils.cs":  # Skip utility file
                functions.append({"name": cs_file.stem, "code": cs_file.read_text()})
            ...
            try:
                ...
                create_button = WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable(
                        (
                            By.CSS_SELECTOR,
                            "button[data-testid='create-function'], button:contains('Create'), button:contains('Add')",
                        )
                    )
                )
                create_button.click()
                ...
                name_field = self.driver.find_element(
                    By.CSS_SELECTOR, "input[name='name'], input[placeholder*='name']"
                )
                name_field.clear()
                ...
                code_field = self.driver.find_element(
                    By.CSS_SELECTOR,
                    "textarea, .code-editor, [data-testid='code-editor']",
                )
                code_field.clear()
                ...
                save_button = self.driver.find_element(
                    By.CSS_SELECTOR,
                    "button[type='submit'], button:contains('Save'), button:contains('Deploy')",
                )
                save_button.click()
    ...
    def verify_deployment(self):
        ...
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "[data-testid='functions-list'], .function-list")
                )
            )

            # Count deployed functions
            function_elements = self.driver.find_elements(
                By.CSS_SELECTOR, "[data-testid='function-item'], .function-item"
            )
