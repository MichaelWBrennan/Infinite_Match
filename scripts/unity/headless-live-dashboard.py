...
class HeadlessLiveDashboard:
    ...
    def get_live_data(self):
        ...
        try:
            ...
            currencies_response = requests.get(currencies_url, headers=self.headers, timeout=5)
            
            inventory_url = f'https://services.api.unity.com/economy/v1/projects/{self.project_id}/environments/{self.env_id}/inventory'
            inventory_response = requests.get(inventory_url, headers=self.headers, timeout=5)
            
            catalog_url = f'https://services.api.unity.com/economy/v1/projects/{self.project_id}/environments/{self.env_id}/catalog'
            catalog_response = requests.get(catalog_url, headers=self.headers, timeout=5)
            
            if currencies_response.status_code == 200:
                data['economy']['currencies'] = currencies_response.json().get('data', [])
        ...
        try:
            ...
            cloud_code_response = requests.get(cloud_code_url, headers=self.headers, timeout=5)
            
            if cloud_code_response.status_code == 200:
                data['cloud_code']['functions'] = cloud_code_response.json().get('data', [])
        ...
        try:
            ...
            remote_config_response = requests.get(remote_config_url, headers=self.headers, timeout=5)
            
            if remote_config_response.status_code == 200:
                data['remote_config']['configs'] = remote_config_response.json().get('data', [])
                
        except Exception as e:
            data['remote_config']['error'] = str(e)
        
        # Determine status
        total_items = (len(data['economy']['currencies']) + 
    ...
    def display_dashboard(self, data):
        ...
        if configs:
            for config in configs[:2]:
                print(f"      • {config.get('name', 'unknown')}: {config.get('value', 'Unknown')}")
        else:
