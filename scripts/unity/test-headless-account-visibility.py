...
class HeadlessAccountVisibilityTester:
            ...
            try:
                with open(self.economy_currencies_path, "r") as f:
                    ...
                    for currency in currencies:
                        print(
                            f"   - {currency.get('id', 'N/A')}: {currency.get('name', 'N/A')} ({currency.get('type', 'N/A')})"
                        )
            except Exception as e:
            ...
            try:
                with open(self.economy_inventory_path, "r") as f:
                    ...
                    for item in inventory:
                        print(
                            f"   - {item.get('id', 'N/A')}: {item.get('name', 'N/A')} ({item.get('type', 'N/A')})"
                        )
            except Exception as e:
        ...
        if self.economy_catalog_path.exists():
            try:
                with open(self.economy_catalog_path, "r") as f:
                    ...
                    for item in catalog:
                        print(
                            f"   - {item.get('id', 'N/A')}: {item.get('name', 'N/A')} ({item.get('cost_currency', 'N/A')}: {item.get('cost_amount', 'N/A')})"
                        )
            except Exception as e:
    ...
    def test_headless_system_capabilities(self):
        ...
        data_management = {
            "read_economy_data": self.economy_currencies_path.exists()
            and self.economy_inventory_path.exists()
            and self.economy_catalog_path.exists(),
        ...
        total_capabilities = (
            len(data_management)
            + len(simulation_capabilities)
            + len(deployment_capabilities)
        )
        active_capabilities = (
            sum(data_management.values())
            + sum(simulation_capabilities.values())
            + sum(deployment_capabilities.values())
        )

        print("✅ Headless System Capabilities:")
        print(
            f"   - Data Management: {sum(data_management.values())}/{len(data_management)} active"
        )
        print(
            f"   - Simulation: {sum(simulation_capabilities.values())}/{len(simulation_capabilities)} active"
        )
        print(
            f"   - Deployment: {sum(deployment_capabilities.values())}/{len(deployment_capabilities)} active"
        )
        print(
            f"   - Total Capabilities: {active_capabilities}/{total_capabilities} active"
        )
    ...
    def save_results(self):
        ...
        results_file = (
            results_dir / f"headless_account_visibility_test_{timestamp}.json"
        )
