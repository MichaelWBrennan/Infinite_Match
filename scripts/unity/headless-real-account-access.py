...
class HeadlessRealAccountAccess:
    ...
    def initialize_services(self):
        ...
        for service in services:
            try:
                ...
                response = requests.post(init_url, headers=self.headers, json={}, timeout=10)
    ...
    def get_cloud_code_data(self):
        ...
        for endpoint in endpoints:
            try:
                ...
                print(f"   Cloud Code ({endpoint.split('/')[-1]}): {response.status_code}")
    ...
    def get_remote_config_data(self):
        ...
        for endpoint in endpoints:
            try:
                response = requests.get(endpoint, headers=self.headers, timeout=10)
                print(f"   Remote Config ({endpoint.split('/')[-1]}): {response.status_code}")
    ...
    def run_real_account_access(self):
        ...
        # Summary
        total_items = (len(economy_data['currencies']) + 
