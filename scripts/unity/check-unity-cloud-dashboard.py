...
class UnityCloudDashboardChecker:
    def __init__(self):
        ...
        self.remote_config_url = f"https://cloud.unity.com/projects/{self.project_id}/remote-config"
        self.cloud_code_url = f"https://cloud.unity.com/projects/{self.project_id}/cloud-code"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "login" in response.url.lower() or "signin" in response.url.lower():
                    ...
                    project_results["project_data"] = self.extract_project_info(html_content)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "login" in response.url.lower() or "signin" in response.url.lower():
                        ...
                        service_result["service_data"] = self.extract_service_data(response.text, service_name)
                        
                elif response.status_code == 404:
                    print(f"❌ {service_name}: Not Found (404) - Service may not be configured")
