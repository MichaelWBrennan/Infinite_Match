...
class HeadlessRealtimeMonitor:
    ...
    def get_economy_data(self):
        ...
        try:
            ...
            return {
                'currencies': currencies_response.json().get('data', []) if currencies_response.status_code == 200 else [],
                'inventory': inventory_response.json().get('data', []) if inventory_response.status_code == 200 else [],
                'catalog': catalog_response.json().get('data', []) if catalog_response.status_code == 200 else [],
                'status': {
    ...
    def get_cloud_code_data(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return {'functions': data.get('data', []), 'status': response.status_code}
            else:
    ...
    def display_data(self):
        ...
        # Summary
        total_items = len(currencies) + len(inventory) + len(catalog) + len(functions) + len(configs)
