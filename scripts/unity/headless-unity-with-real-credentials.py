...
class HeadlessUnityWithCredentials:
    ...
    def login_to_unity(self):
        ...
        WebDriverWait(self.driver, 30).until(EC.url_contains("cloud.unity.com"))
        ...
        try:
            WebDriverWait(self.driver, 20).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "[data-testid='economy'], .economy, .currencies")
                )
            )
            ...
            currencies = []
            try:
                currency_elements = self.driver.find_elements(
                    By.CSS_SELECTOR,
                    "[data-testid='currency'], .currency-item, .currency",
                )
                for element in currency_elements:
                    currency_data = {
                        "id": (
                            element.get_attribute("data-id") or element.text.split()[0]
                            if element.text
                            else "unknown"
                        ),
            ...
            inventory = []
            try:
                inventory_elements = self.driver.find_elements(
                    By.CSS_SELECTOR, "[data-testid='inventory'], .inventory-item, .item"
                )
            ...
            catalog = []
            try:
                catalog_elements = self.driver.find_elements(
                    By.CSS_SELECTOR, "[data-testid='catalog'], .catalog-item, .catalog"
                )
        ...
        try:
            WebDriverWait(self.driver, 20).until(
                EC.presence_of_element_located(
                    (
                        By.CSS_SELECTOR,
                        "[data-testid='cloud-code'], .cloud-code, .functions",
                    )
                )
            )
            ...
            functions = []
            try:
                function_elements = self.driver.find_elements(
                    By.CSS_SELECTOR,
                    "[data-testid='function'], .function-item, .function",
                )
                for element in function_elements:
                    function_data = {
                        "name": (
                            element.get_attribute("data-name")
                            or element.text.split()[0]
                            if element.text
                            else "unknown"
                        ),
            ...
            return {"functions": functions, "source": "real_unity_cloud"}

        except TimeoutException:
            print("⚠️ Cloud Code page not accessible or no data found")
            return {"functions": [], "source": "real_unity_cloud_no_data"}
        ...
        try:
            WebDriverWait(self.driver, 20).until(
                EC.presence_of_element_located(
                    (
                        By.CSS_SELECTOR,
                        "[data-testid='remote-config'], .remote-config, .configs",
                    )
                )
            )
            ...
            configs = []
            try:
                config_elements = self.driver.find_elements(
                    By.CSS_SELECTOR, "[data-testid='config'], .config-item, .config"
                )
            ...
            return {"configs": configs, "source": "real_unity_cloud"}

        except TimeoutException:
            print("⚠️ Remote Config page not accessible or no data found")
            return {"configs": [], "source": "real_unity_cloud_no_data"}
