#!/usr/bin/env python3
"""
Unity Cloud Account Actual Data Reader
Reads what's actually configured in your Unity Cloud account using proper authentication
"""
...
class UnityCloudActualAccountReader:
    def __init__(self):
        ...
        self.cloud_code_url = (
            f"{self.base_url}/cloud-code/v1/projects/{self.project_id}"
        )
    ...
    def print_header(self):
        print("=" * 80)
        print("🔍 UNITY CLOUD ACTUAL ACCOUNT READER")
        print("=" * 80)
    ...
    def read_actual_economy_data(self):
        """Read actual economy data from Unity Cloud API"""
        print("\n💰 Reading ACTUAL Economy Data from Unity Cloud...")

        economy_data = {
            "service": "Economy",
            "method": "unity_cloud_api",
            "status": "unknown",
            "error": None,
            "actual_data": {},
        }
        ...
        try:
            ...
            if currencies_response.status_code == 200:
                ...
                economy_data["actual_data"]["currencies"] = currencies_data
                ...
                print(
                    f"   ❌ Currencies: HTTP {currencies_response.status_code} - {currencies_response.text}"
                )
            ...
            if inventory_response.status_code == 200:
                ...
                economy_data["actual_data"]["inventory"] = inventory_data
                print(f"   ✅ Inventory: Found {len(inventory_data)} items")
                for item in inventory_data:
                    print(f"     - {item.get('id', 'N/A')}: {item.get('name', 'N/A')}")
            else:
                print(
                    f"   ❌ Inventory: HTTP {inventory_response.status_code} - {inventory_response.text}"
                )
            ...
            if catalog_response.status_code == 200:
                ...
                economy_data["actual_data"]["catalog"] = catalog_data
                print(f"   ✅ Catalog: Found {len(catalog_data)} items")
                for item in catalog_data:
                    print(f"     - {item.get('id', 'N/A')}: {item.get('name', 'N/A')}")
            else:
                print(
                    f"   ❌ Catalog: HTTP {catalog_response.status_code} - {catalog_response.text}"
                )

            # Determine status
            if economy_data["actual_data"]:
                economy_data["status"] = "success"
                print("✅ Economy Data: Successfully read from Unity Cloud API")
            else:
        ...
        self.results["actual_account_data"]["economy"] = economy_data
        return economy_data

    def read_actual_remote_config_data(self):
        """Read actual remote config data from Unity Cloud API"""
        print("\n⚙️ Reading ACTUAL Remote Config Data from Unity Cloud...")
        ...
        try:
            ...
            if config_response.status_code == 200:
                ...
                remote_config_data["actual_data"]["configs"] = config_data
                print(f"   ✅ Remote Config: Found {len(config_data)} configurations")
                for config in config_data:
                    print(
                        f"     - {config.get('key', 'N/A')}: {config.get('value', 'N/A')}"
                    )
            else:
                print(
                    f"   ❌ Remote Config: HTTP {config_response.status_code} - {config_response.text}"
                )

            # Determine status
            if remote_config_data["actual_data"]:
                remote_config_data["status"] = "success"
                print("✅ Remote Config Data: Successfully read from Unity Cloud API")
            else:
                remote_config_data["status"] = "no_data"
                remote_config_data["error"] = (
                    "No remote config data found in Unity Cloud"
                )
        ...
        self.results["actual_account_data"]["remote_config"] = remote_config_data
        return remote_config_data

    def read_actual_cloud_code_data(self):
        """Read actual cloud code data from Unity Cloud API"""
        print("\n☁️ Reading ACTUAL Cloud Code Data from Unity Cloud...")
        ...
        try:
            ...
            if functions_response.status_code == 200:
                ...
                cloud_code_data["actual_data"]["functions"] = functions_data
                print(f"   ✅ Cloud Code: Found {len(functions_data)} functions")
                for func in functions_data:
                    print(
                        f"     - {func.get('name', 'N/A')}: {func.get('description', 'N/A')}"
                    )
            else:
                print(
                    f"   ❌ Cloud Code: HTTP {functions_response.status_code} - {functions_response.text}"
                )

            # Determine status
            if cloud_code_data["actual_data"]:
                cloud_code_data["status"] = "success"
                print("✅ Cloud Code Data: Successfully read from Unity Cloud API")
            else:
        ...
        self.results["actual_account_data"]["cloud_code"] = cloud_code_data
        return cloud_code_data

    def generate_actual_account_report(self):
        """Generate actual account report"""
        print("\n" + "=" * 80)
        print("📊 UNITY CLOUD ACTUAL ACCOUNT REPORT")
        print("=" * 80)

        total_services = len(self.results["actual_account_data"])
        successful_services = 0
        no_data_services = 0
        error_services = 0

        for service_name, service_data in self.results["actual_account_data"].items():
        ...
        if successful_services > 0:
            ...
            print("   - This is what's really on your account, not local files")
        else:
    ...
    def save_results(self):
        ...
        results_file = results_dir / f"unity_cloud_actual_account_{timestamp}.json"
    ...
    def run_all_reads(self):
        """Run all Unity Cloud actual account reads"""
        self.print_header()

        # Read actual data from each service
        self.read_actual_economy_data()
        self.read_actual_remote_config_data()
        self.read_actual_cloud_code_data()

        # Generate report
        self.generate_actual_account_report()
...
def main():
    """Main function"""
    reader = UnityCloudActualAccountReader()
