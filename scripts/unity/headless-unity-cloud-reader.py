        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if economy_info:
                    ...
                    print(
                        f"   📊 Found: {len(currencies)} currencies, {len(inventory)} inventory, {len(catalog)} catalog"
                    )
            ...
            if response.status_code == 200:
                ...
                if config_info:
                    ...
                    remote_config_data["error"] = (
                        "No remote config data found in dashboard"
                    )
            else:
    ...
    def generate_headless_report(self):
        ...
        for service_name, service_data in self.results["headless_account_data"].items():
            ...
            if status == "success":
                ...
                print(
                    f"✅ {service_name}: SUCCESS - Data found in Unity Cloud dashboard"
                )
            elif status == "no_data":
                no_data_services += 1
                print(
                    f"❌ {service_name}: NO DATA - Nothing found in Unity Cloud dashboard"
                )
            elif status == "error":
                error_services += 1
                print(
