...
class HeadlessUnityConnectionTester:
        ...
        try:
            ...
            if currencies_path.exists():
                with open(currencies_path, 'r') as f:
                    ...
                    currency_lines = len([line for line in currencies_data.split('\n') if line.strip()])
                    test_result['details']['currencies_count'] = currency_lines - 1  # Subtract header
            ...
            if inventory_path.exists():
                with open(inventory_path, 'r') as f:
                    ...
                    inventory_lines = len([line for line in inventory_data.split('\n') if line.strip()])
            ...
            if catalog_path.exists():
                with open(catalog_path, 'r') as f:
                    catalog_data = f.read()
                    catalog_lines = len([line for line in catalog_data.split('\n') if line.strip()])
    ...
    def test_unity_services_config(self):
        ...
        try:
            config_path = Path('unity/Assets/StreamingAssets/unity_services_config.json')
            ...
            test_result['details'] = {
                ...
                'currencies_count': len(config_data.get('economy', {}).get('currencies', [])),
                'inventory_count': len(config_data.get('economy', {}).get('inventory', [])),
            ...
            if missing_fields:
                ...
                print(f"   Environment ID: {config_data.get('environmentId', 'NOT SET')}")
                print(f"   Cloud Services Available: {config_data.get('cloudServicesAvailable', False)}")
    ...
    def test_headless_scripts(self):
        ...
        if found_scripts == total_scripts:
            ...
            test_result['status'] = 'partial'
            print(f"⚠️ {found_scripts}/{total_scripts} headless automation scripts found")
        else:
            test_result['status'] = 'failed'
    ...
    def test_github_workflow(self):
        ...
        try:
            ...
            found_indicators = [indicator for indicator in headless_indicators if indicator.lower() in workflow_content.lower()]
            ...
            if len(found_indicators) >= 2:  # At least 2 headless indicators
                ...
                test_result['status'] = 'partial'
                print(f"⚠️ GitHub Actions workflow partially configured for headless deployment")
    ...
    def test_simulation_capability(self):
        ...
        try:
            ...
            if unity_service_path.exists():
                ...
                found_indicators = [indicator for indicator in simulation_indicators if indicator in unity_service_content]
    ...
    def generate_recommendations(self):
        ...
        if self.test_results['tests'].get('headless_scripts', {}).get('status') == 'failed':
            recommendations.append({
                'priority': 'high',
                'category': 'Automation',
        ...
        for test_name, result in self.test_results['tests'].items():
            ...
            if result.get('details') and result['details']:
                for key, value in result['details'].items():
                    if key not in ['files', 'scripts', 'indicators']:  # Skip complex nested data
                        print(f"   {key}: {value}")
            print()
