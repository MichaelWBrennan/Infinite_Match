...
class RedundancyCleanup:
        ...
        if cloud_code_dir.exists() and unity_cloud_code_dir.exists():
            for file in unity_cloud_code_dir.glob("*.js"):
                if (cloud_code_dir / file.name).exists():
                    ...
                    file.unlink()
                    self.log_action(
                        "REMOVED", file, f"Duplicate of {cloud_code_dir / file.name}"
                    )
    ...
    def remove_redundant_unity_scripts(self):
        ...
        for pattern in redundant_patterns:
            ...
            if file_path.exists():
                ...
                file_path.unlink()
                self.log_action(
                    "REMOVED", file_path, "Redundant Unity automation script"
                )
    ...
    def remove_redundant_economy_scripts(self):
        ...
        for script in redundant_economy:
            ...
            if file_path.exists():
                ...
                file_path.unlink()
                self.log_action(
                    "REMOVED", file_path, "Redundant economy processing script"
                )
    ...
    def consolidate_duplicate_files(self):
        ...
        for file_path in self.repo_root.rglob("*"):
            if (
                file_path.is_file()
                and file_path.suffix in [".js", ".py", ".md", ".sh"]
        ...
        for duplicate, original in duplicates:
            if duplicate.exists():
                ...
                duplicate.unlink()
                self.log_action(
                    "REMOVED", duplicate, f"Duplicate content of {original}"
                )
