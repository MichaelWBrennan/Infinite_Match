  ...
  "prevention": "How to prevent this error"
    ...
    findSimilarErrors(error, context) {
        return this.errorHistory.filter(historicalError => {
            const messageSimilarity = this.calculateStringSimilarity(error.message.toLowerCase(), historicalError.error.message.toLowerCase());
        ...
        similarErrors.forEach(error => {
            Object.entries(error.context).forEach(([key, value]) => {
                if (!contextCounts[key])
                    contextCounts[key] = {};
                if (!contextCounts[key][value])
                    contextCounts[key][value] = 0;
...
Return JSON with recovery strategy:
{
  ...
  "reasoning": "Why this strategy was chosen"
    ...
    getErrorStats() {
        ...
        const recentErrors = this.errorHistory.filter(error => Date.now() - error.timestamp < 3600000 // Last hour
        );
    ...
    calculateRecoverySuccessRate() {
        const successfulRecoveries = this.errorHistory.filter(error => error.recoveryResult?.success);
...
//# sourceMappingURL=ai-error-handler.js.map