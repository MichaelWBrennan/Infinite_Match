...
class AIContentGenerator {
    ...
    async generatePersonalizedContent(playerId, contentType, preferences) {
        try {
            ...
            switch (contentType) {
                case 'level':
                    content = await this.generateLevel(playerProfile.currentLevel + 1, this.calculateOptimalDifficulty(playerProfile), playerProfile);
                    break;
                case 'event':
                    content = await this.generateEvent(this.selectOptimalEventType(playerProfile), playerProfile.segment, marketTrends);
                    break;
                case 'offer':
