...
class UnifiedRealtimeSystem {
    ...
    async getPlayerRealtimeData(playerId, timezone = 'UTC') {
        try {
            ...
            const weatherData = playerLocation
                ? await this.weatherService.getCurrentWeather(playerLocation.latitude, playerLocation.longitude, playerLocation.name)
                : null;
            // Get calendar events
    ...
    async updatePlayerLocation(playerId, latitude, longitude, locationName = null, timezone = 'UTC') {
        try {
            ...
            const { error } = await this.supabase.from('player_locations').upsert(location);
            if (error)
                throw error;
            // Update cache
            this.playerLocations.set(playerId, {
    ...
    async performHealthCheck() {
        try {
            ...
            const hasIssues = Object.values(health).some((service) => service.status !== 'active' && service.status !== 'healthy');
            if (hasIssues) {
...
//# sourceMappingURL=unified-realtime-system.js.map