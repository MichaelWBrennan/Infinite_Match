...
class PlayerInterventionSystem {
    ...
    initializeActionTemplates() {
        this.actionTemplates.set('immediate_notification', {
            ...
            execute: async (playerId, action, context) => {
                await pushNotificationService.sendNotification(playerId, action.template, { ...action.data, intervention: true, priority: 'high' });
            }
        });
        this.actionTemplates.set('notification', {
            ...
            priority: 'medium',
            execute: async (playerId, action, context) => {
                await pushNotificationService.sendNotification(playerId, action.template, { ...action.data, intervention: true });
            }
        });
    ...
    async giveReward(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`reward:${playerId}:${Date.now()}`, rewardData, 'player_rewards', 86400);
    ...
    async triggerSocialEvent(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`social_event:${playerId}:${Date.now()}`, socialEvent, 'social_events', 86400);
    ...
    async provideHint(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`hint:${playerId}:${Date.now()}`, hintData, 'player_hints', 3600);
    ...
    async givePowerUp(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`power_up:${playerId}:${Date.now()}`, powerUpData, 'player_power_ups', 86400);
    ...
    async adjustDifficulty(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`difficulty_adjustment:${playerId}:${Date.now()}`, difficultyAdjustment, 'difficulty_adjustments', 86400);
    ...
    async createPersonalizedOffer(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`personalized_offer:${playerId}:${Date.now()}`, personalizedOffer, 'personalized_offers', 3600);
    ...
    async providePrioritySupport(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`priority_support:${playerId}:${Date.now()}`, supportRequest, 'priority_support', 86400);
    ...
    async provideSocialRecognition(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`social_recognition:${playerId}:${Date.now()}`, recognition, 'social_recognition', 86400);
    ...
    async recommendContent(playerId, action) {
        try {
            ...
            await aiCacheManager.set(`content_recommendation:${playerId}:${Date.now()}`, recommendation, 'content_recommendations', 3600);
    ...
    calculateOverallRisk(churnAnalysis, progressionAnalysis, socialAnalysis, engagementAnalysis) {
        try {
            ...
            const risk = (churnAnalysis?.risk || 0) * weights.churn +
    ...
    async updateMetrics() {
        try {
            ...
            await aiCacheManager.set('player_intervention_metrics', metrics, 'intervention_metrics', 300);
        }
        catch (error) {
    ...
    async trackActionExecution(playerId, action, intervention) {
        try {
            ...
            await aiCacheManager.set(`action_execution:${playerId}:${Date.now()}`, trackingData, 'action_executions', 86400);
        }
        catch (error) {
...
//# sourceMappingURL=player-intervention-system.js.map