...
class OptimizedCloudServicesManager {
    ...
    async initializeAzureServices() {
        ...
        this.azureClients = {
            blob: new BlobServiceClient(`https://${process.env.AZURE_STORAGE_ACCOUNT}.blob.core.windows.net`, credential),
            keyvault: new SecretClient(`https://${process.env.AZURE_KEYVAULT_NAME}.vault.azure.net`, credential),
            servicebus: new ServiceBusClient(process.env.AZURE_SERVICEBUS_CONNECTION_STRING, credential),
    ...
    async getHealthStatus() {
        const healthChecks = await Promise.allSettled(Array.from(this.healthChecks.entries()).map(async ([name, check]) => {
