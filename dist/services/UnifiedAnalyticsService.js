...
class UnifiedAnalyticsService {
    ...
    getAddictionData(userId) {
        const streakData = this.dailyStreaks.get(userId);
        const activeFOMOEvents = Array.from(this.fomoEvents.values()).filter((event) => event.expiresAt > new Date());
        ...
        return {
            ...
            activeFOMOEvents,
            recentRewards,
            comebackBonuses: Array.from(this.comebackBonuses.values()).filter((bonus) => bonus.userId === userId && bonus.expiresAt > new Date()),
        };
    }
}
// Export singleton instance
export default new UnifiedAnalyticsService();
//# sourceMappingURL=UnifiedAnalyticsService.js.map