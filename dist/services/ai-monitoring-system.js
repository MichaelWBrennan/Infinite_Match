...
class AIMonitoringSystem extends EventEmitter {
    ...
    async processAlert(alert) {
        ...
        // Check rate limit
        const recentAlerts = this.alertHistory.filter(a => a.type === alert.type && Date.now() - a.timestamp < 3600000 // 1 hour
        );
    ...
    async collectDetailedMetrics() {
        try {
            ...
            // Store in Redis
            await this.redis.setex(`ai_metrics:${Date.now()}`, 86400, // 24 hours
    ...
    async storeMetrics() {
        try {
            await this.redis.setex('ai_monitoring:current_metrics', 300, // 5 minutes
...
//# sourceMappingURL=ai-monitoring-system.js.map