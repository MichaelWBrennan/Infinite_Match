...
class PushNotificationService {
        ...
        try {
            ...
            for (const user of targetUsers) {
                for (const notification of campaign.notifications) {
                    ...
                    if (delay > 0) {
                        await this.scheduleNotification(user.id, notification.templateKey, new Date(Date.now() + delay * 1000), { ...notification.customData, campaignId });
                    }
                    else {
                        await this.sendNotification(user.id, notification.templateKey, { ...notification.customData, campaignId });
                    }
                }
            }
    ...
    async applyABTesting(userId, templateKey) {
        try {
            ...
            const activeTests = Array.from(this.abTests.values()).filter(test => test.status === 'running' &&
    ...
    async trackNotificationSent(userId, templateKey, response) {
        try {
            ...
            // Store in analytics
            await aiCacheManager.set(`notification_tracking:${response}`, trackingData, 'notification_analytics', 604800 // 7 days
            );
            this.engagementMetrics.notificationsSent++;
        }
        catch (error) {
    ...
    async updateEngagementMetrics() {
        try {
            ...
            // Store metrics
            await aiCacheManager.set('push_notification_metrics', this.engagementMetrics, 'analytics', 3600);
    ...
    async handleNotificationClick(userId, messageId, action) {
        try {
            ...
            const trackingData = await aiCacheManager.get(`notification_tracking:${messageId}`, 'notification_analytics');
