...
class LiveOpsDashboard {
        ...
        try {
            ...
            if (event.notification) {
                for (const player of targetPlayers) {
                    await pushNotificationService.sendNotification(player.id, event.notification.template, {
                        eventId: eventId,
        ...
        try {
            switch (abTest.type) {
                ...
                case 'campaign':
                    ...
                    break;
                case 'notification':
                    await pushNotificationService.sendNotification(player.id, variant.template, { abTestId: abTest.id, variantId: variant.id });
                    break;
            }
    ...
    async updateRealTimeMetrics(metricName, value, metadata = {}) {
        try {
            ...
            // Store in cache
            await aiCacheManager.set(`live_ops_metric:${metricName}`, metric, 'real_time_metrics', 300 // 5 minutes
            );
        }
        catch (error) {
    ...
    async sendAlertNotifications(alert) {
        try {
            ...
            await aiCacheManager.set(`alert:${alert.id}`, alert, 'alerts', 86400 // 24 hours
            );
    ...
    async updateDashboardMetrics() {
        try {
            ...
            await aiCacheManager.set('live_ops_dashboard_metrics', metrics, 'dashboard_metrics', 60 // 1 minute
            );
        }
        catch (error) {
...
//# sourceMappingURL=live-ops-dashboard.js.map