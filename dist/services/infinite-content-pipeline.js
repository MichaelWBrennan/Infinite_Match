...
class InfiniteContentPipeline {
        ...
        try {
            ...
            const personalizedContentPromises = activePlayers.map(async (playerId) => {
                try {
                    ...
                    this.logger.error(`Failed to generate personalized content for ${playerId}`, {
                        error: error.message,
                    });
                    return null;
                }
    ...
    async generatePlayerSpecificContent(playerId, profile) {
        try {
            ...
            for (const contentType of contentTypes) {
                content[contentType] = await this.aiContentGenerator.generatePersonalizedContent(playerId, contentType, profile.preferences);
            }
            return {
                playerId,
                content,
            ...
            this.logger.error(`Failed to generate player-specific content for ${playerId}`, {
                error: error.message,
            });
            return null;
        }
    ...
    async assessContentQuality(content) {
        try {
            ...
            const overallScore = Object.values(qualityFactors).reduce((sum, score) => sum + score, 0) /
    ...
    async storeBatchContent(batchId, contentType, content) {
        try {
            const { error } = await this.supabase.from('content_batches').insert({
                id: batchId,
    ...
    calculateMarketAlignmentScore(content) {
        return 0.8;
    }
...
//# sourceMappingURL=infinite-content-pipeline.js.map