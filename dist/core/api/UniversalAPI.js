...
export class UniversalAPI {
     ...
     */
    async trackInternalEvent(event) {
        try {
    ...
    async trackEventLocal(eventName, parameters) {
     ...
     * Helper methods
     */
    shouldShowAd(adType, adSettings) {
        if (adType === 'banner')
            return adSettings.banner_enabled;
        if (adType === 'interstitial')
            return adSettings.interstitial_enabled;
        if (adType === 'rewarded')
            return adSettings.rewarded_enabled;
        return false;
    }
    async simulateAdDisplay(adType) {
    ...
    calculateEngagementScore(data) {
        ...
        let score = 0;
        if (sessions > 0)
            score += Math.min(sessions * 10, 50);
        if (avgDuration > 0)
            score += Math.min(avgDuration / 60, 30);
        if (lastDuration > 0)
            score += Math.min(lastDuration / 60, 20);
...
//# sourceMappingURL=UniversalAPI.js.map