...
export class WebGLMiddleware {
     ...
     */
    webglServingMiddleware = async (req, res, next) => {
        try {
     ...
     */
    async serveWebGLFile(req, res, platform) {
        try {
            const filePath = this.getWebGLFilePath(req.path);
            if (!existsSync(filePath)) {
                res
                    .status(404)
                    .json(ApiResponseBuilder.error('WEBGL_FILE_NOT_FOUND', 'WebGL file not found', 'not_found', false, 'check_build_files'));
                return;
            }
            // Get file stats
            ...
            res
                .status(500)
                .json(ApiResponseBuilder.error('WEBGL_SERVE_ERROR', 'Failed to serve WebGL file', 'server_error', true, 'retry_request'));
        }
    }
    /**
     * Get WebGL file path
     */
    getWebGLFilePath(requestPath) {
     ...
     */
    async optimizeForPlatform(req, res, filePath, platform) {
     ...
     */
    setWebGLHeaders(res, filePath, platform) {
     ...
     */
    getCompressionType(req, platform) {
     ...
     */
    async serveCompressedFile(req, res, filePath, compression) {
        try {
     ...
     */
    async serveWebGLConfig(req, res, platform) {
        try {
            const config = {
            ...
            res
                .status(500)
                .json(ApiResponseBuilder.error('WEBGL_CONFIG_ERROR', 'Failed to serve WebGL configuration', 'server_error', true, 'retry_request'));
        }
    }
    /**
     * Handle platform-specific optimization requests
     */
    async handlePlatformOptimization(req, res, platform) {
        try {
            const optimization = {
                platform: platform.name,
                type: platform.type,
     ...
     */
    getOptimizationRecommendations(platform) {
...
//# sourceMappingURL=WebGLMiddleware.js.map