...
// Login endpoint
router.post('/login', security.authRateLimit, validateLogin, asyncHandler(async (req, res) => {
...
// Register endpoint
router.post('/register', security.authRateLimit, validateRegister, async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
