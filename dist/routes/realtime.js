...
router.get('/weather/current', async (req, res) => {
    try {
        ...
        const weather = await weatherService.getCurrentWeather(parseFloat(latitude), parseFloat(longitude), location);
        res.json({
            success: true,
            data: weather,
        });
...
router.get('/weather/forecast', async (req, res) => {
    try {
        ...
        const forecast = await weatherService.getWeatherForecast(parseFloat(latitude), parseFloat(longitude), parseInt(days));
        res.json({
            success: true,
            data: forecast,
        });
...
router.get('/calendar/events', async (req, res) => {
    try {
        ...
        const events = await calendarService.getCalendarEvents(startDate, endDate, timezone, eventTypesArray);
        res.json({
            success: true,
            data: events,
        });
...
router.get('/calendar/upcoming', async (req, res) => {
    try {
        ...
        const events = await calendarService.getUpcomingEvents(parseInt(hours), timezone);
        res.json({
            success: true,
            data: events,
        });
...
router.get('/events/upcoming', async (req, res) => {
    try {
        const { hours = 24, timezone = 'UTC' } = req.query;
        const events = await eventService.getUpcomingEvents(parseInt(hours), timezone);
        res.json({
            success: true,
            data: events,
        });
...
router.post('/events/:eventId/progress', async (req, res) => {
    try {
        ...
        const progress = await eventService.updateEventProgress(eventId, playerId, progressData);
        res.json({
            success: true,
            data: progress,
        });
...
router.post('/unified/player/:playerId/location', async (req, res) => {
    try {
        ...
        await unifiedSystem.updatePlayerLocation(playerId, parseFloat(latitude), parseFloat(longitude), locationName, timezone);
        res.json({
            success: true,
