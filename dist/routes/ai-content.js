...
router.post('/generate', async (req, res) => {
    try {
        ...
        if (playerId && preferences) {
            ...
            content = await aiContentGenerator.generatePersonalizedContent(playerId, contentType, preferences);
        }
        else {
            ...
            switch (contentType) {
                case 'level':
                    content = await aiContentGenerator.generateLevel(req.body.levelNumber || 1, req.body.difficulty || 5, { marketTrends: marketInsights });
                    break;
                case 'event':
                    content = await aiContentGenerator.generateEvent(req.body.eventType || 'daily', req.body.playerSegment || 'casual', marketInsights);
                    break;
                case 'visual':
                    content = await aiContentGenerator.generateVisualAsset(req.body.assetType || 'background', req.body.description || 'Game asset', req.body.style || 'cartoon');
                    break;
                default:
                    return res.status(400).json({
                        success: false,
...
router.get('/recommendations/:playerId', async (req, res) => {
    try {
        ...
        const recommendations = await personalizationEngine.generatePersonalizedRecommendations(playerId, contentType || 'level');
        res.json({
            success: true,
...
router.post('/personalized-offers', async (req, res) => {
    try {
        ...
        const offers = await personalizationEngine.generatePersonalizedOffers(playerId, offerType || 'discount');
        res.json({
            success: true,
            offers: offers,
        });
...
router.post('/analytics/player-behavior', async (req, res) => {
    try {
        ...
        const analysis = await analyticsEngine.analyzePlayerBehavior(playerId, timeRange || '7d');
        res.json({
            success: true,
            analysis: analysis,
        });
...
router.post('/analytics/content-performance', async (req, res) => {
    try {
        ...
        const analysis = await analyticsEngine.analyzeContentPerformance(contentId, contentType);
        res.json({
            success: true,
            analysis: analysis,
        });
...
router.get('/analytics/optimization', async (req, res) => {
    try {
        const { gameArea } = req.query;
        const recommendations = await analyticsEngine.generateOptimizationRecommendations(gameArea || 'general');
        res.json({
            success: true,
            recommendations: recommendations,
        });
...
router.post('/pipeline/generate', async (req, res) => {
    try {
        ...
        const content = await contentPipeline.generateBatchContent(contentType, count || 10);
        res.json({
            success: true,
            content: content,
