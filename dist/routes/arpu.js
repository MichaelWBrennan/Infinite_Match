 ...
 */
router.post('/offers/personalized', security.sessionValidation, [body('playerProfile').isObject().withMessage('Player profile required')], async (req, res) => {
 ...
 */
router.post('/offers/purchase', security.sessionValidation, [
 ...
 */
router.post('/profile/update', security.sessionValidation, [body('updates').isObject().withMessage('Updates object required')], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ success: false, errors: errors.array() });
    }
 ...
 * Track revenue event
 */
router.post('/revenue/track', security.sessionValidation, [
    body('amount').isNumeric().withMessage('Amount must be numeric'),
